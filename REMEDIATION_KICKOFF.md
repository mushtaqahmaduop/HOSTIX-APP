...
> 1. ~~**Brand → HOSTIX.** HOSTYLLO stays reserved for the separate cloud SaaS; DAMAM Boys Hostel and Zeerak Hostix are retired from app chrome.~~ **SUPERSEDED — Mushtaq, 2026-08-30: brand → Hostyllo Offline.** Answer 1 is kept above as the record of what was decided on 2026-07-15, not as current guidance. The desktop app and the cloud SaaS now share one brand, differentiated by edition, because Phase 2 has the desktop fetching its entitlement from the Hostyllo control plane — a customer must not see two vendor names for one purchase. Rename swept in commit `04e73f2`; the identifiers that stay HOSTIX/DAMAM and the reasons they are frozen are tabled in `CLAUDE.md`. **The parenthetical in the original answer — that "DAMAM Boys Hostel" remains valid as a sample default hostel name — is also overruled.** No seeded default may name a real hostel, person or address; the defaults are now neutral (`'Hostel Name'`, empty location).
